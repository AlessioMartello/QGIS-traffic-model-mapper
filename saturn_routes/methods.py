...
#
#
# def unique_codes(routes, volume):
...
def prepare_excel_results(route_codes, all_routes_list, all_volumes, unique_codes_df, route_ids):
    def append(named_list, data):
        named_list.append(data)

    append(all_routes_list,
           unique_codes_df["Routes"])  # List of all routes, before dropping duplicates, used for volume results
    append(all_volumes, unique_codes_df["Volumes"])
    append(route_codes, [route_ids[i].split("/")[-1].strip(".gpkg") for i in range(len(route_ids))])


def create_volume_table(route_codes, unique_codes_list, all_volumes):
    for route_code, links, volumes, names in zip(route_codes, unique_codes_list, all_volumes,
                                                 ["Volumes", "OGV_Volumes"]):

...
for (user_class, data), (volume) in zip(data_sets.items(), volume_data_sets.values()):
    ...
    routes_df=pd.DataFrame(pd.Series(routes), columns=["Routes"])
    routes_df["Volumes"]= pd.Series(volume, index=routes_df.index)
    print(routes_df)

    qgis_routes, route_ids = methods.qgis_json_format(routes, volume) if user_class != "ogv_routes" else methods.qgis_json_format(routes, volume, ogv=True) # Format results
    methods.export_to_json(user_class, qgis_routes) # Export json files

    # Block executes code for the Excek table of results
